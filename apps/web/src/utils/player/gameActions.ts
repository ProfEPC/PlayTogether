 ...
 * @param onVoteUpdate - Callback to record that vote was submitted
 */
export function submitVoteAction(
  ...
  onVoteUpdate: (vote: { value: string }) => void,
) {
  ...
  onVoteUpdate({ value: selectedVote });
...
export function acknowledgeRoleAction(
  ...
  if (!roomState) return;
  socket.emit("player:ackCharacter", {
    roomCode: roomState.roomCode,
    seen: true,
  });
  onStatusUpdate("Acknowledged character");
}
